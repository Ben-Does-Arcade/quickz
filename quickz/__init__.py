...
def ohms_law(v: Value | Phasor = None, i: Value | Phasor = None, r: Value | Phasor = None,
             z: Value | Phasor = None) -> Value:
    calc = None
...
def Xl(f: Value = None, l: Value = None, r: Value = None) -> Value:
    if f is not None and l is not None:
        calc = Value(f"{2 * pi}") * Value(str(f)) * Value(str(l))
        return Value(f"{calc.std.value} Ω")
    elif r is not None:
        return Value(f"{r} Ω")
    else:
